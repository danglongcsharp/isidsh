...
EOD;
	...
	public function htmlDailySchedule($rooms, $date, $out = false){
		...
		$divContent = self::htmlDailyScheduleDiv($rooms, $date);
		ob_start();
		/*
		...
		echo '<tbody>'."\n"
			...
			.'</table>'."\n";
		*/
		echo "<div class='teacher_schedule_div' >";
		echo $divContent;
		echo '</div>';
		$html = ob_get_contents();
		ob_end_clean();
	...
	/** 
	 *	日別スケジュールの時間割の出力: Output timetable in daily schedule board - Div Tags type
	 */
	public function htmlDailyScheduleDiv($rooms, $date, $out = false) {
		...
		for($i=0; $i<count($rooms); $i++) {
			...
			if(!empty($rooms[$i]['url'])) {
				$portrait = '';
				$portrait = apply_filters( 'olb_get_portrait', $portrait, $rooms[$i] );
				$user_profile_html =  get_wp_user_avatar($rooms[$i]['id'], 120);
				$user_profile_html .= "<b>".$rooms[$i]['name']."</b>";
				...
				if (!empty($timetable[$key]['room_id']) && $timetable[$key]['room_id']==$rooms[$i]['id']) {
					...
					if(in_array('past', $trclass)){
						...
						// 開講(予約受付中)
							...
							$class[] = 'open';
							$html .= self::htmlReserveLink($rooms[$i]['id'], $key, $t) ;
						}
					}
					if(!$closestatus)
						$html_detail .= "<div class=\"col-md-6 ".implode(' ', $class)."\">".$html."</div>";
				}
			...
			if($html_detail!=null) {
				$divIndex++;
				echo "<div class=\"col-md-4 user_info_div user_info_div".($divIndex%3)." \">"."<div class='col-md-6 user_info_detail'>".$user_profile_html."</div><div class='col-md-6 user_timeline'>".$html_detail."</div></div>";
			}
